...
```{r echo=FALSE}
output$unsized1 <- renderPlot({
  k = 1
  campca = campca + input$pca1*loadings[[k]][1:25, 1] +
    input$pca2*loadings[[k]][1:25, 2] +
    input$pca3*loadings[[k]][1:25, 3] +
    input$pca4*loadings[[k]][1:25, 4]
  
  
  xs <- campca[seq(1, 22, 2)] %>% as.numeric()
  ys <- campca[seq(2, 22, 2)] %>% as.numeric()
  
  col <- rep(c('red'), each=11)
...
renderUI(
  fluidRow(column(7, plotOutput('unsized1', width = '500px', height = '320px')),
           column(2, plotOutput('unsized2', width = '500px', height = '320px')),
           plotOutput('unsized3', width = '500px', height = '320px'))
)
